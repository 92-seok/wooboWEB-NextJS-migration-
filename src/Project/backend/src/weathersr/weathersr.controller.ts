    ...
    try {
      ...
      return {
        ...
        data: devices.map((item) => ({
          ...
          LOGGER_UPTIME: item.LOGGER_UPTIME,
          LOGGER_TIME: item.LOGGER_TIME,
          LOGGER_GL: item.LOGGER_GL,
          LOGGER_FL: item.LOGGER_FL,
          serviceKey: item.serviceKey,
          observationDateTime: item.observationDateTime,
          waterLevel: item.waterLevel,
          averageVelocity: item.averageVelocity,
          totalDischarge: item.totalDischarge,
          ResultCode: item.ResultCode,
          ResultMsg: item.ResultMsg,
          statusDateTime: item.StatusDateTime,
          statusResultCode: item.StatusResultCode,
          statusResultMsg: item.StatusResultMsg,
